## Assumptions
## 1. Budget for each buy = 100,000.00 Baht
...
##  Buy & Sell Simulation    
    for i in range(1,rows-1):
        if (signal[i-1] == -1) and (signal[i] == 1):
            numBuy[i+1] = floor((100000/(Open[i+1]*1.0017))/100)*100
            num = numBuy[i+1]
            cost[i+1] = -numBuy[i+1]*Open[i+1]*1.0017
            Cost = cost[i+1]
        elif (signal[i-1] == 1) and (signal[i] == -1):
            numSell[i+1] = -num
            income[i+1] = -numSell[i+1]*Open[i+1]*0.9983
